...
def check_hostmask(hostmask):
    rg = re.compile('.*?(!).*?(@).*?', re.IGNORECASE | re.DOTALL)  # regex for checking hostmask format
...
def command_autoop(bot, user, channel, args):
    ...
    if command == 'list':
        ...
        if ops:
            return bot.say(channel, '%s ops: %s' % (list_channel, ', '.join(str(i[0]) for i in ops)))
        ...
        if isAdmin(user):
            ...
            if command == 'add':
                ...
                return bot.say(channel, 'Removing OP failed, is user the user auto-opped?')
        else:
